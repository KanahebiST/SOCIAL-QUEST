...
import { Sparkles, ArrowRight, CheckCircle, ChevronRight, Plus, Shirt, Globe, Flame, Camera, ShieldCheck, Compass } from 'lucide-react';
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  return (
    <div className="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-12 lg:gap-4 animate-fade-in" id="home-view">
      ...
      <div className="lg:col-span-4 space-y-4">
        ...
        <section className="bg-slate-900/90 rounded-2xl pixel-box-gold p-4 sm:p-5 flex flex-col items-center justify-center relative overflow-hidden shadow-xl text-slate-200">
          ...
          <div className="w-full flex items-center justify-between mb-2">
            <span className="px-2.5 py-1 rounded bg-slate-950 text-[10px] font-bold font-pixel text-amber-300 border border-slate-800 shadow-sm flex items-center gap-1">
            ...
            <button
              ...
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 rounded text-[10px] font-bold font-pixel text-emerald-400 border border-slate-700 transition flex items-center gap-1 cursor-pointer pixel-btn"
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 mt-1 mb-3">
            <span className="px-2.5 py-0.5 bg-slate-950 rounded text-[10px] font-bold font-pixel text-amber-300 border border-amber-500/30">
              {user.title || typeResult.title}
            </span>
            <span className="px-2.5 py-0.5 bg-slate-950 rounded text-[10px] font-bold font-pixel text-emerald-400 border border-emerald-500/30">
          ...
          <div className="w-full space-y-2">
            {onOpenQRScanner && (
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold font-pixel text-xs pixel-btn flex items-center justify-center gap-2 shadow-lg"
      ...
      <div className="lg:col-span-8 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          <div className="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl pixel-box text-slate-200">
            <span className="text-[9px] font-bold font-pixel text-slate-400 uppercase tracking-wider block mb-1">
              貢献回数
            </span>
            <div className="flex items-baseline gap-1">
              <span className="text-xl sm:text-2xl font-bold font-press-start text-slate-100">
                {user.contributions.length}
              </span>
              <span className="text-xs font-pixel text-slate-400">回</span>
            </div>
            <span className="text-[10px] font-pixel text-slate-400 block mt-0.5">
              社会貢献アクション
            </span>
          </div>

          {/* Card 2: QR Verified */}
          <div className="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl pixel-box-emerald text-slate-200">
            <span className="text-[9px] font-bold font-pixel text-emerald-400 uppercase tracking-wider block mb-1">
              QR実機認証
            </span>
            <div className="flex items-baseline gap-1">
              <span className="text-xl sm:text-2xl font-bold font-press-start text-emerald-400">
                {verifiedRecycleCount}
              </span>
              <span className="text-xs font-pixel text-emerald-300">本</span>
            </div>
            <span className="text-[10px] font-pixel text-emerald-300 block mt-0.5">
              実機リサイクル
            </span>
          </div>
          ...
          <div className="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl pixel-box text-slate-200">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-lg sm:text-xl font-bold font-press-start text-amber-400">
                {user.xp}
              </span>
              <span className="text-xs font-pixel text-amber-300">XP</span>
            </div>
            <span className="text-[10px] font-pixel text-slate-400 block mt-0.5">
              累積経験値
            </span>
          ...
          <div className="bg-slate-900/90 p-3 sm:p-3.5 rounded-xl pixel-box text-slate-200">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-xl sm:text-2xl font-bold font-press-start text-rose-400 flex items-center gap-0.5">
                🔥{user.streakDays}
              </span>
              <span className="text-xs font-pixel text-slate-400">日連続</span>
            </div>
            <span className="text-[10px] font-pixel text-slate-400 block mt-0.5">
              毎日の継続
            </span>
        ...
        {todaysMission && (
          <section className="bg-slate-900 pixel-box-gold rounded-2xl p-4 sm:p-5 text-slate-200 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative overflow-hidden">
            <div className="relative z-10 flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[9px] font-bold font-press-start px-2 py-0.5 rounded bg-amber-950 border border-amber-500/50 text-amber-300 flex items-center gap-1">
              ...
              <h4 className="text-base sm:text-lg font-bold font-pixel text-slate-100 leading-tight">
                {todaysMission.title}
              </h4>
              <p className="text-slate-300 text-xs font-pixel mt-1 leading-relaxed max-w-lg">
              ...
              <div className="mt-3 flex items-center gap-3">
                <button
                  ...
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 px-4 py-2 rounded-xl font-bold font-pixel text-xs pixel-btn flex items-center gap-1.5"
            ...
            {/* Reward Box */}
            <div className="relative z-10 bg-slate-950 p-3 rounded-xl border border-slate-800 text-center min-w-[100px] shrink-0 self-stretch sm:self-auto flex flex-col justify-center">
              <span className="text-[9px] font-bold font-press-start text-amber-400">
                報酬
              </span>
              <span className="text-lg sm:text-xl font-bold font-press-start mt-0.5 text-amber-300">
